...
import { ActivityIndicator, Text, View, StyleSheet, TextInput, FlatList, TouchableOpacity } from 'react-native';
    ...
    getPrice(materialId, amount, mode = 1) {
        try {
            if (mode == 1) {
                ...
                if (this.state.prices && this.state.prices[materialId]) {
                    let result = this.state.prices[materialId] * amount
                    return result;
                }
            ...
            <View style={{ flex: 1 }}>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => {
                        ...
                        let itemPrice = this.getPrice(item, 1, 0)
                        return (
                            <TouchableOpacity style={styles.cardContainer}>
                                <Text style={{ textAlign: "center" }}>Name: {forgedata[item].id.toLowerCase().replace(/_/g, ' ')}</Text>
                                <Text style={{ textAlign: "center" }}>Craft price: {formatNumber(totalValue)}</Text>
                                <Text style={{ textAlign: "center" }}>Market price: {this.getPrice(item, 1)}</Text>
                                <Text style={{ textAlign: "center" }}>Profit: {formatNumber(itemPrice - totalValue)}</Text>
                                <Text style={{ textAlign: "center" }}>Duration: {forgedata[item].duration} hour(s) ({formatNumber((itemPrice - totalValue) / forgedata[item].duration)} per hour)</Text>
                                <Text></Text>
                                <FlatList
